...
export default function SupplyTypesPage() {
  const [items, setItems] = useState<SupplyType[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [editing, setEditing] = useState<SupplyType | null>(null);
  const [toDelete, setToDelete] = useState<SupplyType | null>(null);
  ...
  const fetchItems = async () => {
    try {
      const res = await apiGet("/products/supply-types/");
      const data = Array.isArray(res.data) ? res.data : res.data?.results ?? [];
      setItems(data);
    } catch {
      showToast("Failed to fetch supply types", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (item: SupplyType) => { setEditing(item); setForm({ name: item.name ?? item.label, label: item.label ?? item.name, is_active: item.is_active }); setModalOpen(true); };
  const openDelete = (item: SupplyType) => { setToDelete(item); setDeleteModalOpen(true); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (editing) {
        await apiPut(`/products/admin/supply-types/${editing.id}/`, form);
        showToast("Supply type updated successfully", "success");
      } else {
        await apiPost("/products/admin/supply-types/create/", form);
        showToast("Supply type created successfully", "success");
      }
      setModalOpen(false);
      fetchItems();
    } catch {
      showToast(`Failed to ${editing ? "update" : "create"} supply type`, "error");
    }
  };
  ...
  const handleDelete = async () => {
    if (!toDelete) return;
    try {
      await apiDelete(`/products/admin/supply-types/${toDelete.id}/`);
      showToast("Supply type deleted successfully", "success");
      setDeleteModalOpen(false);
      setToDelete(null);
      fetchItems();
    } catch {
      showToast("Failed to delete supply type", "error");
    }
  };
  ...
  return (
    <AdminLayout>
      <Head>
       <title>Dashboard — Supply Types</title>
      </Head>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-title-md font-bold text-gray-800 dark:text-white/90">Supply Types</h1>
        <Button onClick={openCreate} startIcon={<PlusIcon />}>Add Supply Type</Button>
      </div>
        ...
        <div className="max-w-full overflow-x-auto">
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {items.length === 0 ? (
                <TableRow>
                  <td colSpan={4} className="px-5 py-8 text-center text-sm text-gray-400">No supply types found</td>
                </TableRow>
              ) : items.map((item, i) => (
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} className="max-w-md">
        <div className="p-5">
          <h2 className="mb-4 text-lg font-bold text-gray-800 dark:text-white/90">
            {editing ? "Edit Supply Type" : "Add Supply Type"}
          </h2>
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              ...
              <InputField
                ...
                placeholder="Enter supply type name"
              />
            </div>
            <div className="mb-5">

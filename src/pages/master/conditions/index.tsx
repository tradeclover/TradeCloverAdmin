...
export default function ConditionsPage() {
  ...
  const fetchItems = async () => {
    try {
      const res = await apiGet("/products/conditions/");
      const data = Array.isArray(res.data) ? res.data : res.data?.results ?? [];
      setItems(data);
    } catch {
      showToast("Failed to fetch conditions", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (item: Condition) => { setEditing(item); setForm({ name: item.name ?? item.label, label: item.label ?? item.name, is_active: item.is_active }); setModalOpen(true); };
  const openDelete = (item: Condition) => { setToDelete(item); setDeleteModalOpen(true); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (editing) {
        await apiPut(`/products/admin/conditions/${editing.id}/`, form);
        showToast("Condition updated successfully", "success");
      } else {
        await apiPost("/products/admin/conditions/create/", form);
        showToast("Condition created successfully", "success");
      }
      setModalOpen(false);
      fetchItems();
    } catch {
      showToast(`Failed to ${editing ? "update" : "create"} condition`, "error");
    }
  };
  ...
  const handleDelete = async () => {
    if (!toDelete) return;
    try {
      await apiDelete(`/products/admin/conditions/${toDelete.id}/`);
        ...
        <div className="max-w-full overflow-x-auto">
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {items.length === 0 ? (
                <TableRow>
                  <td colSpan={4} className="px-5 py-8 text-center text-sm text-gray-400">No conditions found</td>
                </TableRow>
              ) : items.map((item, i) => (

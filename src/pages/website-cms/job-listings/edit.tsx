...
const EditJobListingPage = () => {
    ...
    const fetchJob = async (jobId: string) => {
        try {
            ...
            const response = await apiGet(`/cms/jobs/${jobId}/`);
            ...
            setFormData({
                ...
                opening_valid_till: job.opening_valid_till
            });
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!slug || typeof slug !== 'string') {
            showToast('Invalid job slug', 'error');
            return;
        }
        ...
        try {
            await apiPut(`/cms/jobs/${slug}/update/`, formData);
            showToast('Job listing updated successfully', 'success');

            // Redirect to job listings list after successful update
            setTimeout(() => {
                router.push('/website-cms/job-listings');
            }, 1500);
            ...
            showToast('Failed to update job listing', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <AdminLayout>
            <div className="flex justify-between items-center mb-6">
                <h1 className="font-bold text-gray-800 text-title-md dark:text-white/90">Edit Job Listing</h1>
                <Button
                    variant="outline"

...
const EditPostPage = () => {
    ...
    const fetchPost = async (postId: string) => {
        try {
            ...
            const response = await apiGet(`/cms/posts/${postId}`);
            const post: Post = response.data;

            const normalizedFeatureImageUrl = normalizeMediaUrl(post.feature_image_url || '');
            const normalizedDetailImageUrl = normalizeMediaUrl(post.detail_image_url || '');

            setFormData({
                title: post.title,
                post_type: post.post_type,
                post_category: post.post_category.toString(),
                feature_image_url: normalizedFeatureImageUrl,
                detail_image_url: normalizedDetailImageUrl,
                post_content: post.post_content || '',
                date: post.date,
                most_read: (post as any).most_read || false,
                meta_title: post.meta_title || '',
                meta_description: post.meta_description || '',
                meta_keywords: post.meta_keywords || ''
            });
            setFeatureImagePreview(normalizedFeatureImageUrl);
            setDetailImagePreview(normalizedDetailImageUrl);
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!slug || typeof slug !== 'string') {
            showToast('Invalid post slug', 'error');
            return;
        }
        ...
        try {
            ...
            await apiPut(`/cms/posts/${slug}/update/`, submitData);
    ...
    return (
        <AdminLayout>
            <div className="flex justify-between items-center mb-6">
                <h1 className="font-bold text-gray-800 text-title-md dark:text-white/90">Edit Post</h1>
                <Button
                    variant="outline"

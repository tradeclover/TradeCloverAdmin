...
const UnverifiedUsersPage = () => {
    ...
    const [profileCompletionFilter, setProfileCompletionFilter] = useState<string>('100');
    ...
    const fetchUnverifiedUsers = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await apiGet('/users/admin/unverified-users/');
    ...
    // Filter users based on profile completion
    const filteredUsers = users.filter((user) => {
        if (profileCompletionFilter === 'all') {
            return true;
        }
        return user.profile_completion_percent === 100;
    });
    ...
    return (
        <AdminLayout>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <h1 className="font-bold text-gray-800 text-title-md dark:text-white/90">Unverified Users</h1>
                <div className="inline-flex rounded-lg bg-gray-100 p-1 dark:bg-gray-800">
                    <button
                        onClick={() => setProfileCompletionFilter('100')}
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${
                            profileCompletionFilter === '100'
                    ...
                    <button
                        onClick={() => setProfileCompletionFilter('all')}
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${
                            ...
                            {filteredUsers.length === 0 ? (
                                ...
                                filteredUsers.map((user) => (
                                    <TableRow key={user.id}>
                                        ...
                                        <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                                            <button
                                                onClick={() => router.push(`/unverified-users/${user.id}`)}
